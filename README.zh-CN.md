...
**Sentinel Skill** —— 云端 Agent 下发一个任务,由**端侧 agent** 修复一次真实生产故障(`shop` 里 `payment-api`
CrashLoopBackOff)。`guard skill solve` 在**端侧自主跑有界只读调查 loop**,回传脱敏证据与根因(缺少环境变量);
修复是变更操作 → **readonly 模式拒绝自动执行、门控在人工审批后** → 审批后以 `auto` 模式应用 → pod 恢复 Running。
全程批量删除仍被拦截。
